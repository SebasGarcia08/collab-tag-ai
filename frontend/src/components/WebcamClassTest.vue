<template>
  <div class="float-center md:float-center">
    ...
    <div class="flex items-center justify-center" v-for="(i, idxI) in Math.ceil(classes.length / 3)" :key="idxI">
      <div class="p-1" v-for="(pjClass, idxJ) in classes.slice((i - 1) * 3, i * 3)" :key="idxJ">
        <button class="py-2 w-32 px-4 bg-blue-500 hover:bg-blue-600 rounded-md text-white" 
        @click="handleButton(idxJ + (idxI * 3))"> {{ pjClass }} [{{ classCounter[idxJ + (idxI * 3)] }}] </button>
      </div>
    </div>
